...
-- Q1: Vendor with most Domestic installations
SELECT v.vendor_name AS "Name", u.usage_type AS "Installation Type"
FROM Vendor v
...
-- Q2: Area with highest capacity solar panel
SELECT u.address AS "Name", MAX(sp.capacity) AS "Max Capacity"
FROM Solar_Panel sp
JOIN Installation i ON sp.panel_id = i.panel_id
JOIN User u ON i.user_id = u.user_id
GROUP BY u.address
HAVING MAX(sp.capacity) = (SELECT MAX(capacity) FROM Solar_Panel);

-- Q3: Areas using Monocrystalline panels
SELECT u.address AS "Area", sp.panel_type AS "Pv_type"
FROM Solar_Panel sp
JOIN Installation i ON sp.panel_id = i.panel_id
JOIN User u ON i.user_id = u.user_id
WHERE sp.panel_type = 'Monocrystalline';

-- Q4: Total installation charges in Bangalore
SELECT SUM(sp.cost) AS "Total_Installation_Charges", u.address AS "Area"
FROM Solar_Panel sp
JOIN Installation i ON sp.panel_id = i.panel_id
JOIN User u ON i.user_id = u.user_id
...
-- Q5: Vendor details for earliest installation
SELECT v.vendor_name AS "Name", sp.panel_id AS "pv_module",
       sp.cost AS "price", sp.panel_type AS "pv_type",
       sp.capacity AS "capacity", i.installation_date AS "installation_date"
FROM Vendor v
JOIN Installation i ON v.vendor_id = i.vendor_id
JOIN Solar_Panel sp ON i.panel_id = sp.panel_id
WHERE i.installation_date = (SELECT MIN(installation_date) FROM Installation);

-- Q6: Avg cost of panel types used in Commercial installations
SELECT sp.panel_type AS "Installation Type",
